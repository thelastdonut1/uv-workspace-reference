...
The demo app fetches the top Hacker News headlines. The scraping logic lives in a shared library that two front-ends consume:

```
.
├── pyproject.toml          # workspace root (virtual — not a package itself)
├── uv.lock                 # ONE lockfile for the whole workspace
└── packages/
    ├── core/               # shared library: fetches headlines (httpx + bs4)
    │   ├── pyproject.toml
    │   └── src/core/
    ├── cli/                # CLI front-end (typer + rich), depends on core
    │   ├── pyproject.toml
    │   └── src/cli/
    └── api/                # HTTP front-end (fastapi + uvicorn), depends on core
        ├── pyproject.toml
        └── src/api/
```
...
In `packages/cli/pyproject.toml` (and likewise `api`), `core` is declared as a normal dependency, then `[tool.uv.sources]` tells uv to resolve it from the workspace instead of PyPI:

```toml
[project]
dependencies = [
    "core",                 # ordinary dependency declaration...
    "rich>=15.0.0",
    "typer>=0.26.8",
]
...
Workspace members are installed as *editable*, so changes to `packages/core/src/` are picked up immediately by `cli` and `api` — no reinstall needed.
...
- The API reads `PORT` from the environment (via `python-dotenv`, loaded from a `.env` in the working directory), defaulting to 8080.
